...
function loadPermalink() {
  ...
  try {
    if (/^#md3=/.test(window.location.hash)) {
      cfg = JSON.parse(mdurl.decode(window.location.hash.slice(5), mdurl.decode.componentChars));
    } else if (/^#md64=/.test(window.location.hash)) {
      cfg = JSON.parse(window.atob(window.location.hash.slice(6)));
    } else if (/^#md=/.test(window.location.hash)) {
      cfg = JSON.parse(decodeURIComponent(window.location.hash.slice(4)));
    } else {
      return;
    }
